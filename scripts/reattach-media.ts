...
function replaceMarkers(
  ...
  node.children = node.children.map((child) => {
    const text =
      child?.children?.length === 1 ? child.children[0]?.text : undefined;
...
async function main() {
  ...
  const manifest: Manifest = JSON.parse(
    await readFile("./data/media-manifest.json", "utf8"),
  );
    ...
    try {
      const created = await payload.create({
        ...
        data: {
          alt:
            f.title ||
            filename.replace(/\.[a-z]+$/i, "").replace(/[-_]+/g, " "),
        },
  ...
  for (const rec of editorial) {
    ...
    const merged = current.map((existing, i) => {
      ...
      return {
        ...existing,
        image: media ?? (existing.image as unknown) ?? undefined,
      };
    });
