...
const makeToLexical =
  (editorConfig: EditorConfig, dropped: unknown[]) => (html: string) => {
    const state = convertHTMLToLexical({
      editorConfig,
      html,
      // The converter's type expects `new (html: string) => { window: { document: Document } }`;
      // jsdom's real constructor has optional args and returns DOMWindow, so it does
      // not match structurally even though it behaves correctly.
      JSDOM: JSDOM as unknown as new (html: string) => {
...
async function rescuePerson(
  ...
  const res = await fetch(
    `https://api.themoviedb.org/3/person/${tmdbId}?api_key=${key}`,
  );
...
async function main() {
  ...
  const articles: ArticleRecord[] = JSON.parse(
    await readFile("./data/articles.json", "utf8"),
  );
  ...
  for (const rec of editorial) {
    ...
    const podcasts = rec.podcasts
      .filter((p) => {
        ...
        report.podcastsSkipped.push(
          `${rec.title}: ${p.episode_title ?? p.title ?? "(untitled)"}`,
        );
        return false;
      })
      .map((p) => ({
  ...
  if (!DRY) {
    await writeFile(
      "./data/article-images-todo.json",
      JSON.stringify(imageBacklog, null, 2),
    );
    await writeFile(
...
`);
  ...
  if (report.rescuedPeople.length) {
    console.log(
      `Fetched from TMDB because they were absent from the catalogue:`,
    );
